...
YOLOv8-nano & Deep learning (CNN, pretrained) & None (uses public COCO weights) & Classifying generic obstacles (table, person, etc.) \\
...
\node[proc, right=1.6cm of orb] (nav) {RouteGraph +\\Guided Navigator};
...
\subsection{Guided Navigation — Planned Route With a Reactive Safety Check}
\label{sec:guided-nav}
Without an IMU or wheel odometry, the system cannot do continuous geometric path planning the way SLAM-based navigation does. Instead, it plans at a coarser, discrete level: at the start of a navigation session, the system localizes the user's current position and queries \texttt{RouteGraph} (Dijkstra over previously trained \texttt{WaypointEdge} records, authored via the \texttt{edge} command in \texttt{train.py}) for a sequence of edges connecting the current location to the destination. \texttt{GuidedNavigator} then follows that route step by step:

\begin{itemize}[leftmargin=1.4cm]
  \item Each route step carries a \texttt{direction\_hint} (\textit{forward}, \textit{turn\_left}, \textit{turn\_right}, \textit{turn\_around}) and a spoken \texttt{audio\_instruction}, both authored during training (the \texttt{edge} command in \texttt{train.py}).
  \item A \textit{forward} step is announced as ``go straight'' only if the live ToF center zone actually confirms the path is clear right now — the depth sensor is a standing safety check against a stale plan, never bypassed.
  \item Turn steps are announced directly from the trained hint.
  \item Reaching a step's target waypoint (recognized via ORB) advances to the next step; reaching the final destination ends the session.
\end{itemize}

If no route exists between the recognized starting location and the destination — because no edges were trained between them, or because the starting position could not be confidently recognized — navigation falls back automatically to pure reactive wall-following: the forward view is split into three vertical zones (left, center, right), and the system turns toward whichever side has more clearance when the center is blocked. This fallback is always available and never fails outright; it is simply less directed than a planned route.

This is combined with two independently loaded ORB recognizers: a \emph{location} recognizer (checks for arrival at the destination and for reaching intermediate route steps) and a \emph{landmark} recognizer (announces trained features such as doors when they are recognized close ahead). The planned route only ever covers locations explicitly connected during training — it is not a continuous map and does not generalize to unvisited layouts, a deliberate, honest trade-off documented further in Section~\ref{sec:future}.
...
\begin{enumerate}[leftmargin=1.4cm]
  ...
  \item \textbf{YOLOv8-nano} (convolutional neural network, deep learning) — a pretrained, COCO-trained object detector (80 classes, $\sim$6MB) used to classify what triggered a ToF obstacle alert (e.g. ``table'', ``person''), turning a generic alert into a specific one.
...
\begin{itemize}[leftmargin=1.4cm]
  ...
  \item 33 automated unit tests covering the data model, sensor processing, navigation decision logic, and DL integration's graceful degradation.
\end{itemize}

\subsection{Engineering Challenges}
This section focuses on the three challenges that most shaped the project's architecture and process, rather than a full bug list (several smaller hardware integration fixes are documented in the project's change history).
...
\subsubsection{Challenge 3: Gating Deep Learning Inference Behind a Cheap Check}
